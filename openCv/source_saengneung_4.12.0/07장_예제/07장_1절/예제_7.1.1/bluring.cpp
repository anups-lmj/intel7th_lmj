...
int main()
{
	...
	Mat mask(3, 3, CV_32F, data1);
	Mat mask2(5, 5, CV_32F, data2);
	Mat blur;
	Mat blur1;
	filter(image, blur, mask);
	filter(image, blur1, mask2);
	blur.convertTo(blur, CV_8U);
	blur1.convertTo(blur1, CV_8U);

	cout << "image" << image(Rect(0, 0, 5, 5)) << endl;
	cout << "image" << blur(Rect(0, 0, 5, 5)) << endl;

	imshow("image", image), imshow("blur", blur), imshow("blur1",blur1);
	waitKey();
	return 0;
}