#include <opencv2/opencv.hpp>
using namespace cv;
using namespace std;
int  main()
{
	Mat  image(300, 500, CV_8UC3, Scalar(255,255,255));
	Point2f  center(250, 150), pts[4];
	Size2f  size(300, 100);
	RotatedRect  rot_rect(center, size, 20);
	
	Rect bound_rect = rot_rect.boundingRect();
	rectangle(image, bound_rect, Scalar(0), 1);
	circle(image, rot_rect.center, 1, Scalar(0), 2);

	rot_rect.points(pts);
	for (int i = 0; i < 4; i++)
	{
		circle(image, pts[i], 4, Scalar(0), 1);
		line(image, pts[i], pts[(i + 1) % 4], Scalar(0), 2);
		cout << "pts[" << i << "] : " << pts[i] << endl;
	}

	Point p1, p2, p3, p4;
	p1.x = ((pts[1].x - pts[0].x) / 2) + pts[0].x;
	p1.y = ((pts[0].y - pts[1].y) / 2) + pts[1].y;
	p2.x = ((pts[2].x - pts[3].x) / 2) + pts[3].x;
	p2.y = ((pts[3].y - pts[2].y) / 2) + pts[2].y;

	p3.x = bound_rect.tl().x;
	p3.y = center.y;
	p4.x = bound_rect.br().x;
	p4.y = center.y;
	
	line(image, p1, p2, Scalar(0,0,255), 2);
	line(image, p3, p4, Scalar(0, 255, 0), 2);


	imshow("회전사각형", image);
	waitKey(0);
	return 0;
}